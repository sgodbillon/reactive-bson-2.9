...
trait DefaultBSONHandlers {
  ...
  class BSONArrayCollectionReader[M[_], T](implicit cbf: CanBuildFrom[M[_], T, M[T]], reader: BSONReader[_ <: BSONValue, T]) extends BSONReader[BSONArray, M[T]] {
    def read(array: BSONArray) = {
      val builder = cbf()
      builder ++= array.stream.filter(_.isSuccess).map { v =>
  ...
  class BSONArrayCollectionWriter[T, Repr](implicit repr2traversable: Repr => Traversable[T], writer: BSONWriter[T, _ <: BSONValue]) extends VariantBSONWriter[Repr, BSONArray] {
    def write(repr: Repr) = {
      new BSONArray(repr.map(s => Try(writer.write(s))).toStream)
    }
  }

  implicit def collectionToBSONArrayCollectionWriter[T, Repr](implicit repr2traversable: Repr => Traversable[T], writer: BSONWriter[T, _ <: BSONValue]): VariantBSONWriter[Repr, BSONArray] = {
  ...
  private type BN = BSONNull.type
  private type BU = BSONUndefined.type
